...
//电压状态信号
//128x128的字大小 2048字节
void LCD_ShowHz_128_huan(u16 x,u16 y,u8 num,u8 mode)
{  							  
    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;     
        ...
        
	{
	    ...
	    {   
           ...
           temp=Hz_128x128[num][t];	    //显示哪一个汉字，在数组中找                     
...
//显示冒号  96x48
void LCD_ShowHz_maohao(u16 x,u16 y,u8 num,u8 mode)
{  							  
    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;     
        ...
        
	{
	    ...
	    {   
     		...
     		temp=Hz_96x48_maohao[0][t];	    //显示哪一个汉字，在数组中找                     
...
/*
void Show64x64num(u16 x,u16 y,u8 num,u8 mode)
{
	    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;      			     
...
void Show128x64num(u16 x,u16 y,u8 num ,u8 mode)
{
	    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;   
	...
	else//叠加方式
	{
	    ...
	    {   
			//if(size==12)temp=asc2_1608[num][t];  //调用1206字体
			 temp=num128x128[num][t];		 //调用1608字体 	                          
	        for(t1=0;t1<8;t1++)
			{			    
		        if(temp&0x80) LCD_DrawPoint(x,y); 
				temp<<=1;
...
void Show128x64fuhao(u16 x,u16 y,u8 mode)
{
	    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;   
...
void Show96x48VS(u16 x,u16 y,u8 num,u8 mode)
{
	    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;   
...
//格式化显示数字
void LCD_printfnum128(u16 x,u16 y,u16 width,u16 height,char *fmt,...)
{
	u8 x0;
	
	unsigned char lcdbuff[296];
	
    va_list ap;
	unsigned char *pStr = lcdbuff;
	
	va_start(ap, fmt);
	vsprintf((char *)lcdbuff , fmt, ap);							//格式化
	va_end(ap);
    ...
    {       
        if(x>=width){x=x0;y+=128;}
        if(y>=height)break;            //退出
       // LCD_ShowChar(x,y,*pStr,size,0);
		Show128x64num(x,y,*pStr,0);
        x+=64;
        pStr++;
    }  
}
...
//液晶格式化打印字符
void LCD_printf(u16 x,u16 y,u16 width,u16 height,u8 size, char *fmt,...)
{
    u8 x0;
	unsigned char lcdbuff[296];
...
void LCD_ShowHz_48(u16 x,u16 y,u8 num,u8 mode)
{  							  
    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;     
        ...
        
	{
	    ...
	    {   
           ...
           temp=Hz_48x48[num][t];	    //显示哪一个汉字，在数组中找                     
...
void LCD_ShowHz_96x48(u16 x,u16 y,u8 num,u8 mode)
{  							  
    u16 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;     
        ...
        
	{
	    ...
	    {   
  			...
  			temp=Hz96x48[0][t];	    //显示哪一个汉字，在数组中找                     
